export const createCardDOM = ({currentUserId, template, data, delButton, onLike, imageClick}) => {
  ...
  image.addEventListener('click', () =>
    imageClick({cardName: data.name, cardLink: data.link})
  );
  image.src = data.link;
  image.alt = data.name;
  ...
  if (data.owner._id === currentUserId) {
    ...
    deleteButton.addEventListener('click', () => {
      delButton({cardId: data._id, cardElement: element, buttonElement: deleteButton
      });
    });
  }
  ...
  likeButton.addEventListener('click', () => {
    onLike({cardId: data._id, buttonElement: likeButton, counterElement: counter
    });
  });
